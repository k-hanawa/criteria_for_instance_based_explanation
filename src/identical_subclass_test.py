...
def main():
    ...
    model = get_model_fn(args.model, setting, saved_dir, num_class=2)
    ...
    train_labels = [(str(d[2]), int(d[1])) for d in train]
    ...
    for test_idx in tqdm(test_idxs):
        ...
        for met_func, met_name in zip(met_funcs, met_names):
            ...
            all_results.append({'idx': int(test_idx), 'y': int(target_data[1]), 'z': str(target_data[2]),
