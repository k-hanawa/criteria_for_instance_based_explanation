...
def main():
    ...
    train_loader, validation_loader = mobilenetv2.make_dataloaders(data_params, merge=True)
    train_loader_y, validation_loader_y = mobilenetv2.make_dataloaders(data_params, merge=False)

    data_params = {'path': 'data/cifar10_torchvision_merged', 'batch_size': 32}
    train_loader_batch, validation_loader_batch = mobilenetv2.make_dataloaders(data_params, merge=True)

    data_params = {'path': 'data/cifar10_torchvision_merged', 'batch_size': 1}
    train_loader_raw, validation_loader_raw = mobilenetv2.make_dataloaders(data_params, raw=True, merge=True)
    ...
    train_y = [[d for d in train_loader_y][i] for i in train_idxs]
    ...
    model = mobilenetv2.mobilenet_v2(pretrained=True, num_classes=2, device=device,
                                     model_path=os.path.join(args.saved_dir, 'best_model.pt'))
    model.out_dir = saved_dir

    model = model.to(device).eval()
    model.device = device

    zs = [int(d[1][0]) for d in train_y]
    ys = [int(d[1][0]) for d in train]
    train_labels = [(z, y) for z, y in zip(zs, ys)]
    ...
    for test_idx in tqdm(test_idxs):
        ...
        for inf_func, inf_name in zip(inf_funcs, inf_names):
            ...
            all_results.append({'idx': int(test_idx), 'z': int(target_data_y[1][0]), 'y': int(target_data[1][0]), 'met': inf_name, 'scores': np.array(infs)})
