...
import src.torch.mobilenetv2 as mobilenetv2
...
def main():
    ...
    train_loader, validation_loader, _ = mobilenetv2.make_dataloaders(data_params)

    data_params = {'path': 'data/cifar10_torchvision', 'batch_size': 32}
    train_loader_batch, validation_loader_batch, _ = mobilenetv2.make_dataloaders(data_params)

    data_params = {'path': 'data/cifar10_torchvision', 'batch_size': 1}
    train_loader_raw, validation_loader_raw, _ = mobilenetv2.make_dataloaders(data_params, raw=True)
    ...
    preds = []
    with torch.no_grad():
        for image, label in tqdm(train_loader_batch):
            image = image.to(model.device)
            prediction = model(image)
            preds.append(np.argmax(prediction.cpu().numpy(), axis=1))

    preds = np.vstack([np.hstack(preds)[i] for i in train_idxs])
    ...
    for test_idx in tqdm(test_idxs):
        ...
        target_data_raw = train_raw[test_idx]
